...
- **בצע מיד** — כשהבקשה ברורה, כתוב קוד, commit ו-push ללא שלב תכנון נפרד.
- **שאל לפני ביצוע** — רק אם חסר מידע קריטי שבלעדיו אי אפשר לממש נכון.
- **הצג תכנון** — רק אם המשתמש מבקש זאת במפורש, או אם השינוי נוגע ל-3+ קבצים עם סיכון רגרסיה גבוה.
- **סיכום קצר** — אחרי כל שינוי: מה בוצע + רשימת בדיקות.

> חריג — תיקון באג ברור בשורה בודדת, ללא שום ספק לגבי הכוונה: מותר תמיד עם הסבר קצר.
...
## תוכניות בשיקול (לא לבצע בלי אישור)

- **מעבר מ-Firebase לדרייב + סנכרון אוטומטי לכל המכשירים** — `docs/plan-drive-sync.md`.
  כולל את הבעיות בשיטה הנוכחית (דריסת היסטוריה ב-`saveArchiveToCloud`, מפתחות
  וקודי PIN גלויים, משיכה ידנית), השלבים וההחלטות הפתוחות. לקרוא לפני כל שינוי
  ב-`FirebaseManager` או בלוגיקת השמירה לענן.
