...
הסקיל נטען בכל סשן דרך `.claude/hooks/session-start.sh`, ולכן הוא ברירת המחדל
ולא בקשה חד-פעמית. סייגים שגוברים עליו, כולם מ-`CLAUDE.md`:

- **קידום גרסה בשלושה מקומות** (`version.json`, `sw.js`, `CURRENT_VERSION` ב-`script.js`)
  בכל commit שמשנה קבצי אפליקציה. חיסכון אינו עילה לדלג.
- **ניתוח השפעה לפני מחיקה או refactor.** "עריכה כירורגית" אינה היתר לקצר או
  למחוק קוד "מת".
- **בסוף כל שינוי משמעותי: מה בוצע, רשימת בדיקות (User Flows), ו"מוכן להוראה"** —
  "Silent completion" חל על שלבי ביניים, לא על הדיווח הסופי.
- **וידוא שה-auto-merge ל-`main` רץ** לפני שמדווחים שהעדכון הגיע.
